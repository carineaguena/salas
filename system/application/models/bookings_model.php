...
class Bookings_model extends Model{
  ...
  function BookingCell($data, $key, $rooms, $users, $room_id, $url, $booking_date_ymd = '', $holidays = array()){
  	...
  	if(isset($data[$key])){
		...
		else
		{
  		...
  		$cell['class'] = 'free';
  		$cell['body'] = '<a href="'.$book_url.'"><img src="webroot/images/ui/accept.gif" width="16" height="16" alt="Book" title="Book" hspace="4" align="absmiddle" />Agendar</a>';
			if($this->userauth->CheckAuthLevel(ADMINISTRATOR, $this->authlevel)){
  ...
  function html($school_id = NULL, $display = NULL, $cols = NULL, $date = NULL, $room_id = NULL, $school, $uri = NULL){
			...
			if($display == 'room'){
		  	...
		  	switch($display){
		  		case 'room':
		  			...
		  			$week_bar['back_text'] = '&lt;&lt;Semana Anterior';
		  			...
		  			$week_bar['next_text'] = 'Próxima Semana&gt;&gt;';
		  			$week_bar['next_link'] = sprintf('bookings/index/date/%s/room/%s/direction/next', $week_bar['next_date'], $room_id);
		  			$week_bar['longdate'] = 'Semana começando em '.date("l jS F Y", strtotime($this_week->date));
		  		break;
		  		case 'day':
				  	...
				  	if(date("Y-m-d") == date("Y-m-d", $date)){
							$week_bar['back_text'] = '&lt;&lt; Ontem';
				  		$week_bar['next_text'] = 'Amanhã &gt;&gt; ';
				  	} else {
				  		$week_bar['back_text'] = '&lt;&lt; Voltar';
				  		$week_bar['next_text'] = 'Próximo &gt;&gt; ';
				  	}
		  		break;
		  	}
  	...
  	if($query->num_rows() > 0){
  		...
  		$html .= $this->load->view('msgbox/error', 'Não há períodos disponíveis. Por favor, fale com o administrador.', True);
  		#return $html;
  		$err = true;
  	}
  	...
  	if(!isset($periods)){
  		$html .= $this->load->view('msgbox/warning', 'Não há períodos configurados para essa semana. Por favor, escolha outra data.', True);
  		return $html;
  	}
