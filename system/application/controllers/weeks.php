...
class Weeks extends Controller {
  ...
  function index(){
  	...
  	#$body .= $this->load->view('weeks/weeks_index_academicyear', $view, True);
		...
		$layout['title'] = 'Tabela de Ciclo Semanal';
	...
	function delete(){
	  // Get ID from URL
		...
		if( $this->input->post('id') ){
			...
			$body['id'] = $id;
			$body['cancel'] = 'weeks';
			$body['text'] = 'Se essa semana for deletada, <strong>todos os agendamentos</strong> feitos nessa semana serão<strong> permanentemente apagados</strong>.';
			// Load page
			$row = $this->M_weeks->Get($id, $this->school_id);
			$layout['title'] = 'Deleta Semana ('.$row->name.')';
			$layout['showtitle'] = $layout['title'];
	...
	function saveacademicyear(){
		...
		// Pretty it up a bit for error validation message
		$vfields['date_start']		= 'Data de início';
		$vfields['date_end']			= 'Data de fim';
		$this->validation->set_fields($vfields);
