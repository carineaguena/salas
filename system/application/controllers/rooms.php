  ...
  function info(){
	 ...
	 */
	 function save(){
		...
		$vfields['room_id']					= 'Sala ID';
		$vfields['name']						= 'Nome da Sala';
		$vfields['location']				= 'Localização';
		$vfields['user_id']					= 'Professor';
		$vfields['icon']						= 'Icon';
		$vfields['notes']						= 'Notas';
		$vfields['bookable']				= 'Pode ser agendada';
		$vfields['photo']						= 'Foto';
		$vfields['photo_delete']		= 'Apagar foto';
    ...
    if ($this->validation->run() == FALSE){
      ...
      // Validation failed
			...
			if( !$this->upload->do_upload() ){
				...
				if( $error != 'Você não selecionou nenhum arquivo para upload' ){
	...
	function delete(){
	  // Get ID from URL
		...
		if( $this->input->post('id') ){
			...
			$body['id'] = $id;
			$body['cancel'] = 'rooms';
			$body['text'] = 'Se você deletar esta sala, <strong>todos os agendamentos</strong> para esta sala serão <strong>permanentemente apagados</strong> também.';
			// Load page
	...
	function fields_index(){
		...
		$body['fields'] = $this->M_rooms->GetFields($this->session->userdata('schoolcode'));
		// Set main layout
		$layout['title'] = 'Campos de Sala';
		$layout['showtitle'] = 'Define campos para salas';
	...
	function fields_edit($id = NULL){
	 ...
	 function fields_save(){
		...
		// Pretty it up a bit for error validation message
		$vfields['field_id']		= 'Campo ID';
		$vfields['name']				= 'Nome do Campo';
		$vfields['items']				= 'Itens';
		$this->validation->set_fields($vfields);
	...
	function fields_delete(){
	  // Get ID from URL
		...
		if( $this->input->post('id') ){
			...
			$this->session->set_flashdata('saved', $this->load->view('msgbox/info', 'O campo foi deletado.', True) );
			// Redirect to rooms again
			redirect('rooms/fields', 'redirect');
		} else {
			// Initialise page
			...
			$layout['title'] = 'Deletar campo ('.$row->name.')';
			$layout['showtitle'] = $layout['title'];
			$layout['body'] = $this->load->view('partials/deleteconfirm', $body, TRUE);
			$this->load->view('layout', $layout);
		}
