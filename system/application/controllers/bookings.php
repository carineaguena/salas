...
class Bookings extends Controller {
  ...
  function load(){
  	...
  	#$chosen_date = $this->input->post('chosen_date');
		...
		$vfields['chosen_date']		= 'Data';
		$vfields['room_id']				= 'Sala';
		$vfields['direction']			= 'Direção';
    	...
    	switch($style['display']){
    		case 'day':
		    	...
		    	if($this->input->post('chosen_date')){
						...
						show_error('nenhuma data escolhida');
					}
				break;
				case 'room':
	...
	function book(){
		...
		if($seg_count != 2 && $seg_count != 12){
			...
			// Not all info in URI
			$layout['body'] = $this->load->view('msgbox/error', 'Não há informação suficiente para agendar uma sala.', True);
			...
			if($this->userauth->CheckAuthLevel(TEACHER, $this->authlevel)){
				if($thedate < $today){
					$layout['body'] = $this->load->view('msgbox/error', 'Você não pode agendar uma sala no passado.', True);
				}
			}
			...
			if($this->userauth->CheckAuthLevel(TEACHER, $this->authlevel)){
				...
				if ($bia > 0) {
					...
					if($thedate > $date_forward){
						$layout['body'] = $this->load->view('msgbox/error', 'Você só pode agendar'.$bia.' dias posteriores.', True);
					}
	...
	function recurring(){
		...
		if($errcount > 0){
			$flashmsg = $this->load->view('msgbox/error', 'Um ou mais agendamentos não podem ser feitos.', True);
		} else {
			$flashmsg = $this->load->view('msgbox/info', 'Os agendamentos foram criados com sucesso.', True);
		}
	...
	function cancel(){
		...
		if($this->M_bookings->Cancel($this->school_id, $booking_id)){
			$msg = $this->load->view('msgbox/info', 'Os agendamentos foram <strong>cancelados</strong>.', True);
		} else {
			$msg = $this->load->view('msgbox/error', 'Um erro ocorreu no cancelamento do agendamento.', True);
		}
      ...
      // Validation failed
			...
			if($booking_id == 'X'){
				...
				if(!$this->M_bookings->Add($data)){
					...
					$flashmsg = $this->load->view('msgbox/info', 'O agendamento foi realizado.', True);
				}
			} else {
				...
				if(!$this->M_bookings->Edit($booking_id, $data)){
					...
					$flashmsg = $this->load->view('msgbox/info', 'O agendamento foi atualizado.', True);
				}
			} // End of booking_id=X
	...
	function callback__is_valid_date($date){
		...
		if(count($datearr) == 3){
			...
			if($valid){
				...
				$ret = false;
				$this->validation->set_message('_is_valid_date', 'Data inválida');
			}
		} else {
			$ret = false;
			$this->validation->set_message('_is_valid_date', 'Data inválida');
